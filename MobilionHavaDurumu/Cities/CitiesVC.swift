...
class CitiesVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        oldArray = userDefaults.stringArray(forKey: selectedCitiesUserDefaults) ?? [""]
        let selectedCitiesArray = userDefaults.stringArray(forKey: selectedCitiesUserDefaults)
        if selectedCitiesArray != nil {
...
extension CitiesVC: UITableViewDelegate, UITableViewDataSource {
    ...
    /*
     Kullanıcının sildiği şehirlere göre UserDefaults güncellenir.
     */
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if (editingStyle == .delete) {
            ...
            userDefaults.set(self.oldArray, forKey: selectedCitiesUserDefaults)
            self.tableView.reloadData()
        }
    }
}
